...
class TestSettings(unittest.TestCase):
    ...
    @patch(
        ...
        return_value={"key1": 500, "key2": 600, "key3": "val3", "key4": "val4"},
    )
    ...
    @patch(
        ...
        read_data='{"key1": 500, "key2": 600, "key3": "val3", "key4": "val4"}',
    )
    @patch("src.settings.logging.getLogger")
    @patch("src.settings.sg.popup")
    @patch(
    ...
    @patch("src.settings.pathlib.Path.absolute", return_value="dummy_path")
    def test_save(
        self, mock_abs, mock_jsonload, mock_popup, mock_logger, mock_open, mock_jsondump
    ):
        s = Settings()
