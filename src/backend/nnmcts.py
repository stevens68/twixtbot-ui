...
class NeuralMCTS:
    ...
    def visit_node(self, game, node, top=False, trials=None):
        ...
        if top:
            self.logger.debug(
                "selecting index=%d move=%s Q=%.3f P=%.5f N=%d",
                index,
                str(move),
                node.Q[index],
                node.P[index],
    ...
    def _scew(self, p):
        ...
        prob = [
            (-4 * x + 2 * avg) * q * q + (4 * x - 3 * avg) * q + avg for x in prob[1:]
        ]
        prob.insert(
            0, (-4 * p0 + 2 * avg + 2) * q * q + (4 * p0 - 3 * avg - 1) * q + avg
        )
        return prob
    ...
    def create_response(
        ...
        if not moves:
            ...
            resp["moves"] = [naf.policy_index_point(game.turn, i) for i in indices]
    ...
    def mcts(self, game, trials, window, event):
        ...
        if self.root is None:
            ...
            if self.logger.level <= logging.INFO:
                ...
                for ix in top_ixs:
                    msg += (
                        f"{naf.policy_index_point(game, ix)}: "
