...
class TwixtBoard:
    ...
    def valid_spot(self, move, game):

        p = self._move_to_point(move)
        if len(game.history) >= 2 and game.history[1] == twixt.SWAP:
            # game is swapped
    ...
    def get_move(self, game, coords):
        ...
        if not self.valid_spot(move, game):
            return None
