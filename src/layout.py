...
def popup(message):
    sg.popup_quick_message(
        message, keep_on_top=True, line_width=200, auto_close_duration=3
    )
...
def row_turn_indicators():
    bg_col = ct.OUTPUT_BACKGROUND_COLOR
    return [
        text_label(ct.K_TURN_INDICATOR[0]),
        sg.Input(
            size=(14, 1),
            disabled_readonly_background_color=bg_col,
            readonly=True,
            key=ct.K_TURN_INDICATOR[1],
...
def row_names():
    return [
        text_label(ct.K_NAME[0]),
        text_output(ct.K_NAME[1], 14),
        pad(1),
...
def row_auto_moves():
    return [
        text_label(ct.K_AUTO_MOVE[0]),
        sg.Checkbox(
            text="",
            enable_events=True,
            default=False,
            key=ct.K_AUTO_MOVE[1],
            size=(7 + ct.OFFSET, 1),
        ),
        sg.Checkbox(
            text="",
...
def row_trials():
    return [
        text_label(ct.K_TRIALS[0]),
        sg.Slider(
            range=(0, ct.TRIALS_MAX),
            default_value=0,
            size=(11, 20),
...
def row_progress_bar():
    return [text_label(ct.K_PROGRESS_BAR[0]), get_progress_bar()]


def row_progress_nums():
    return [
        text_label(ct.K_PROGRESS_NUM[0]),
...
def row_visualize_mcts():
    return [
        text_label(ct.K_VISUALIZE_MCTS[0]),
        sg.Checkbox(
            text="",
            enable_events=True,
...
def row_heatmap():
    return [
        text_label(text=ct.K_HEATMAP[0]),
        sg.Checkbox(
            "",
            enable_events=True,
            default=False,
...
def row_eval_moves():
    bg_col = ct.OUTPUT_BACKGROUND_COLOR
    return [
        text_label(ct.K_EVAL_MOVES[0]),
        sg.Canvas(size=(240, 80), background_color=bg_col, key=ct.K_EVAL_MOVES[1]),
    ]
...
def row_eval_hist():
    bg_col = ct.OUTPUT_BACKGROUND_COLOR
    return [
        text_label(ct.K_EVAL_HIST[0]),
        sg.Canvas(size=(240, 80), background_color=bg_col, key=ct.K_EVAL_HIST[1]),
    ]
...
class MainWindowLayout:
    ...
    def build_layout(self):
        menu_def = [
            [
                ct.ITEM_FILE,
                [ct.ITEM_OPEN_FILE, ct.ITEM_SAVE_FILE, ct.ITEM_SETTINGS, ct.ITEM_EXIT],
            ],
        ...
        layout = [
            ...
            [board_col, eval_bar_col, control_col],
            button_row,
        ]
...
def st_row_allow_swap():
    return [
        st_label(ct.K_ALLOW_SWAP[0]),
        sg.Checkbox(text="", default=ct.K_ALLOW_SWAP[3], key=ct.K_ALLOW_SWAP[1]),
    ]


def st_row_allow_scl():
    return [
        st_label(ct.K_ALLOW_SCL[0]),
        sg.Checkbox(text="", default=ct.K_ALLOW_SCL[3], key=ct.K_ALLOW_SCL[1]),
    ]


def st_row_smart_accept():
    return [
        st_label(ct.K_SMART_ACCEPT[0]),
        sg.Checkbox(text="", default=ct.K_SMART_ACCEPT[3], key=ct.K_SMART_ACCEPT[1]),
    ]


def st_row_resign_threshold():
    return [
        st_label(ct.K_RESIGN_THRESHOLD[0]),
        sg.Spin(
            values=[float((x + 70.0) / 100.0) for x in range(31)],
            initial_value=ct.K_RESIGN_THRESHOLD[3],
            key=ct.K_RESIGN_THRESHOLD[1],
            size=(5, 0),
            readonly=True,
        ),
    ]
...
def st_row_auto_move(player):
    return [
        st_label(ct.K_AUTO_MOVE[0]),
        sg.Checkbox(
            text="", default=ct.K_AUTO_MOVE[player + 2], key=ct.K_AUTO_MOVE[player]
        ),
    ]
...
def st_row_temperature(player):
    return [
        st_label(ct.K_TEMPERATURE[0]),
        sg.Combo(
            ct.TEMPERATURE_LIST,
            ct.K_TEMPERATURE[player + 2],
            size=(5, 1),
            key=ct.K_TEMPERATURE[player],
            readonly=True,
        ),
    ]
...
def st_row_add_noise(player):
    return [
        st_label(ct.K_ADD_NOISE[0]),
        sg.Spin(
            values=[float(x / 100.0) for x in range(101)],
            initial_value=ct.K_ADD_NOISE[player + 2],
            key=ct.K_ADD_NOISE[player],
            size=(5, 0),
            readonly=True,
        ),
    ]
...
def st_row_cpuct(player):
    return [
        st_label(ct.K_CPUCT[0]),
        sg.Spin(
            values=[float(x / 100.0) for x in range(101)],
            initial_value=ct.K_CPUCT[player + 2],
            key=ct.K_CPUCT[player],
            size=(5, 0),
            readonly=True,
        ),
    ]
...
def st_row_rotation(player):
    return [
        st_label(ct.K_ROTATION[0]),
        sg.Combo(
            ct.ROTATION_LIST,
            ct.K_ROTATION[player + 2],
            size=(15, 1),
            key=ct.K_ROTATION[player],
            readonly=True,
        ),
    ]
...
def st_row_level(player):
    return [
        st_label(ct.K_LEVEL[0]),
        sg.Spin(
            values=[float(x / 100.0) for x in range(101)],
            initial_value=ct.K_LEVEL[player + 2],
            key=ct.K_LEVEL[player],
            size=(5, 0),
            readonly=True,
        ),
    ]
...
def st_row_smart_root(player):
    return [
        st_label(ct.K_SMART_ROOT[0]),
        sg.Checkbox(
            text="", default=ct.K_SMART_ROOT[player + 2], key=ct.K_SMART_ROOT[player]
        ),
    ]
    ...
    def build_layout():
        ...
        st_tab_general = [
            ...
            [
                ...
                sg.Checkbox(
                    text="", default=ct.K_SHOW_LABELS[3], key=ct.K_SHOW_LABELS[1]
                ),
            ],
            [
                ...
                sg.TabGroup(
                    [
                        [
                            ...
                            sg.Tab(ct.TAB_LABEL_PLAYER1, st_tab_player1, pad=(20, 0)),
                            sg.Tab(ct.TAB_LABEL_PLAYER2, st_tab_player2, pad=(10, 0)),
                        ]
                    ]
                )
            ],
...
def build_layout() -> object:
    ...
    return [
        ...
        [sg.Text("", size=s)],
        [sg.Text("twixtbot-ui frontend by stevens68 and contributors", size=s)],
