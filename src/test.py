...
def demo_one_line_progress_meter():
    # Display a progress meter. Allow user to break out of loop using cancel
    # button
...
def custom_meter_example():
    ...
    for i in range(10000):
        # check to see if the cancel button was clicked and exit loop if
        # clicked
        event, values = window.read(timeout=0)
        if event == 'Cancel' or event == None:
            break
        # update bar with loop value +1 so that bar eventually reaches the
        # maximum
...
def demo_iterable_progress_bar():
    ...
    # Since the progress_bar is an iterator, you can use it within a list
    # comprehension
